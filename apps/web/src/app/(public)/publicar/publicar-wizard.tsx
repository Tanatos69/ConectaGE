...
interface FormData {
  ...
  /** Only shown/sent for QUANTITY_CATEGORIES; empty string = not set. */
  quantity: string;
  country: string;
  region: string;
  city: string;
...
function categoryIcon(iconName: string | null): typeof iconByName[string] {
  return (iconName && iconByName[iconName]) || iconByName.faBoxOpen;
}
...
export function PublicarWizard({
  ...
  const currentSubcategories = subcategoriesByParent[form.category] ?? [];
    ...
    <div className="bg-muted/30">
      ...
      <div className="mx-auto max-w-2xl px-4 py-6 space-y-5">
              ...
              {form.category === "" ? (
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {categories.map((cat) => (
                    ...
                    >
                      <FontAwesomeIcon icon={categoryIcon(cat.icon)} className="size-6" aria-hidden="true" />
                      <span>{cat.name}</span>
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="inline-flex items-center gap-2 text-sm font-semibold text-foreground">
                      {(() => { const ic = categories.find((c) => c.slug === form.category); return ic ? <FontAwesomeIcon icon={categoryIcon(ic.icon)} className="size-4" aria-hidden="true" /> : null; })()}
                      {form.categoryName}
                    </span>
                  </div>
                    ...
                    {currentSubcategories.map((sub) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground">
                          {subcategoryCounts.get(`${form.category}:${sub.slug}`) ?? 0}
                        </span>
                      </button>
                    ))}
                  </div>
        ...
        {step === 3 && (
          <SectionCard
            title={
              extraFields.length > 0 || showQuantity
                ? `Especificaciones de ${form.categoryName}`
          ...
          <div className="space-y-4">
            <SectionCard title="Resumen de tu anuncio">
              <dl className="space-y-3 text-sm">
                ...
                {showQuantity && form.quantity && (
                  <div className="flex justify-between gap-2">
                    <dt className="text-muted-foreground">Cantidad disponible</dt>
                    <dd className="font-medium text-foreground">{form.quantity}</dd>
                  </div>
                )}
                <div className="flex justify-between gap-2">
